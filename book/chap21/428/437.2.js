...
const betterCoefficients = new Proxy(coefficients, { // 가로챌때 기동하는 로직
  // Get trap
  // 객체의 프로퍼티의 값을 가져가려 할 때 기동하는
  // 트랩(미리 사용처가 정해진 프락시 로직용 함수)객체 입니다.
  get(target, key) {
    return target[key] || 0;
  },
});

let result = evaluate(5, betterCoefficients); // NaN
